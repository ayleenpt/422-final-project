...
STRELOAD_MAX	EQU		0x00FFFFFF		; MAX Value = 1/16MHz * 16M = 1 second
STCURR_CLR		EQU		0x00000000		; Clear STCURRENT and STCTRL.COUNT	
SIGALRM			EQU		14				; SIGALRM signal

; System Variables
SECOND_LEFT		EQU		0x20007B80		; Seconds left for alarm( )
USR_HANDLER     EQU		0x20007B84		; Address of user-defined signal handler	
...
_timer_init
    LDR     R1, =STCTRL
    MOV     R0, #STCTRL_STOP  ; Stop SysTick (CLK_SRC=1, INT_EN=0, ENABLE=0)
    STR     R0, [R1]

    LDR     R1, =STRELOAD
    LDR     R0, =STRELOAD_MAX  ; Load max value for 1-second timer
    STR     R0, [R1]
...
_timer_start
    ...
    LDR     R1, =SECOND_LEFT
    STR     R0, [R1]       ; Store countdown time

    ; Enable SysTick Timer with Interrupts
    LDR     R1, =STCTRL
    MOV     R0, #STCTRL_GO ; ENABLE=1, INT_EN=1, CLK_SRC=1
    STR     R0, [R1]
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Timer update (called by SysTick_Handler)
; void timer_update( )
...
_timer_update
    ...
    CMP     R0, #0
    BLE     _timer_update_done  ; If already 0, skip decrement
    ...
    LDR     R1, =USR_HANDLER
    LDR     R1, [R1]        ; Load handler address
    CMP     R1, #0
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Signal Handler (Stores User Function Pointer)
; void* signal_handler( int signum, void* handler )
		EXPORT	_signal_handler
_signal_handler
    PUSH    {R4, LR}

    CMP     R0, #SIGALRM   ; Ensure sig is SIGALRM (14)
    BNE     _signal_handler_done  ; If not, do nothing

    LDR     R2, =USR_HANDLER
    LDR     R0, [R2]    ; Load previous function pointer (return value)
    STR     R1, [R2]    ; Store new function pointer in USR_HANDLER
